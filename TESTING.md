...
| Fixtures from the real box | `dashboard/tests/fixtures/` | does the parser survive the output the actual commands print, `[N/A]` and all? |
...
| `cockpit.py` | 68% | 2,898 lines at v1.18.7; the auth, CSRF, static, action, job and diagnostics surfaces are covered, the sampler threads are not |
...
| module | floor | why it is where it is |
|---|---|---|
| `keepalive-proxy.py` | 83% | the production serving path; the rest is socket teardown |
...
- Fixtures are **real output from the reference box**, trimmed, not invented.
  An `[N/A]` from `nvidia-smi` on unified memory, the engine's log tail and the
  docker, systemctl and proxy journal outputs are in `dashboard/tests/fixtures/`;
  a French `free -g` and a `Received output for rid=...` flood line are written
  inline in the tests that feed them (`test_cockpit_collectors.py`,
  `test_lifecycle.py`).
