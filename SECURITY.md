...
Two paths still put the engine itself on the network: `ENGINE_BIND=0.0.0.0
./install.sh`, and `./run.sh`, the foreground path of `--no-service`, which
serves it on `0.0.0.0:30000` with no proxy at all. On either, anything on the
LAN or the tailnet that holds the key can reach the engine directly and end it,
exactly as it could before these guards existed: the key is the boundary there,
not the proxy. Nothing is lost by the default: the proxy is a full pass-through of
