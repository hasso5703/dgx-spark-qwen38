...
class Qwen3_5GatedDeltaNet(nn.Module):
    ...
    def forward(
        ...
        # Power-of-two ratios are supported everywhere; the ratio-3 dense 27B
        # layout is handled by the CUDA Triton kernel's per-head walk only
        # (the CPU fused op still requires a power-of-two group).
        use_fused_contiguous_unpack = value_to_key_head_ratio in [1, 2, 4, 8] or (
            value_to_key_head_ratio == 3 and _is_cuda
        )
