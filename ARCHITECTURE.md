...
| `keepalive-proxy.py` | the only component that reads bytes it does not control | keepalive at SSE event boundaries, zombie abort ordering (`x-override-rid`, abort *before* close, drain otherwise), oversize guard that counts via `/tokenize` and prices images from their headers, corruption tripwire; the `v6.x` header is its real version, read live by the cockpit; since v6.19 it answers `POST /v1/systemone` itself (typed decisions read off `top_logprobs`, one single-token branch per question, never `token_ids_logprob`: that path kills this build's scheduler on a mixed batch, sglang#34719) |
