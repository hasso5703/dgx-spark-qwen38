...
HEAD = '  if [ "$LANE" = "27b" ] && [ -r "$SGL_UNIT_PATH" ]; then\n    _INSTALLED_CTX='
...
def guard() -> str:
    ...
    i = INSTALL.find(HEAD)
    assert i != -1, "the native-restore guard is gone from install.sh (or its opening changed)"
    j = INSTALL.find(TAIL, i)
    assert j != -1, "the guard no longer sits above the 27B restore block"
    block = INSTALL[i:j]
    assert SENTINEL in block, "the guard no longer warns"
    return block


def run_guard(unit_text: str, *, lane="27b", no_service=1, no_start=0, tmp=None):
    unit = tmp / "unit"
...
class Guard(unittest.TestCase):
    ...
    def test_it_warns_for_no_start_too(self):
    ...
    def test_it_names_both_ways_out(self):
        ...
        self.assertIn("--no-service/--no-start", out)
